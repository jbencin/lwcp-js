...
describe('LWCP.Parser', () => {
    ...
    // Compare javascript value to LWCP.Value. Necessary to handle arrays
    let compareValue = (v: LWCP.Value, a: any) => {
        if (Array.isArray(v.val)) {
            expect(v.val.length).toBe(a.length);
            for (let j in v.val) {
                compareValue(v.val[j], a[j]);
            }
        } else {
            expect(v.val).toBe(a);
        }
    ...
    describe('Parser.parseMessage()', () => {
        ...
        it(`Test: ${str}`, () => {
            ...
            compareValue(msg.props[0].val, '555-1234');
            expect(msg.props[1].name).toBe('hybrid');
            compareValue(msg.props[1].val, 'false');
        })
    })
